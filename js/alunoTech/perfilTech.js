...
async function loadBoletimAndMetrics() {
    ...
    // Lógica do seletor de foco da Média (Qual trimestre mostrar na última coluna)
    const selFoco = document.getElementById('al-sel-media-foco');
    const trimestreFoco = selFoco ? selFoco.value : '1';
    
    // Adiciona o evento de mudança apenas uma vez para recalcular ao alterar o trimestre
    ...
    // Atualiza o título da última coluna da tabela no HTML para refletir o foco atual
    const thMediaFinal = els.boletimBody.closest('table').querySelector('thead tr th:last-child');
    if (thMediaFinal) {
        if (trimestreFoco === 'geral') thMediaFinal.innerText = 'Média Final';
        else thMediaFinal.innerText = `Média ${trimestreFoco}º Tri`;
    }
    ...
    for (const [discId, isEnrolled] of Object.entries(mapDisciplinas)) {
        ...
        // Coleta e calcula as médias matematicamente e individualmente para os 3 trimestres
        ...
        // Define o que será exibido baseado na seleção do usuário
        let calcFinal = null;
        ...
        // Se existir um cálculo válido para a seleção atual, renderiza a nota e as cores
        if (calcFinal !== null) {
            mediaExibida = calcFinal.toFixed(1);
            // Ajuste o 6.0 para 7.0 caso seja a nota de corte do colégio
            bgMedia = calcFinal >= 6.0 ? "bg-green-900/20 text-green-400" : "bg-red-900/20 text-red-400";
        }

        // Mantida a estrutura exata de 14 colunas
        html += `<tr class="bg-slate-900/10 hover:bg-slate-900/50 transition-colors">
            ...
            <td class="media-final-col font-bold border border-slate-700 text-center py-2 ${bgMedia} transition-all duration-300">${mediaExibida}</td>
        </tr>`;
    }
