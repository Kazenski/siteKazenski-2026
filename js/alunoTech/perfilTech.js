...
async function loadBoletimAndMetrics() {
    ...
    // Salva globalmente para os gráficos
    ...
    // Identifica o seletor atual da tabela
    ...
    for (const [discId, isEnrolled] of Object.entries(mapDisciplinas)) {
        ...
        // Calcula matematicamente todas as médias
        ...
        const fmt2 = m2 !== null ? m2.toFixed(1) : "---";
        const fmt3 = m3 !== null ? m3.toFixed(1) : "---";
        const fmtGeral = mGeral !== null ? mGeral.toFixed(1) : "---";

        // Define a exibição inicial baseada no que o select está marcando no momento da carga
        let mediaExibida = "---";
        if (trimestreFoco === '1') mediaExibida = fmt1;
        else if (trimestreFoco === '2') mediaExibida = fmt2;
        else if (trimestreFoco === '3') mediaExibida = fmt3;
        else if (trimestreFoco === 'geral') mediaExibida = fmtGeral;

        // Configuração visual de cores
        let bgMedia = "bg-blue-900/10 text-blue-400";
        if (mediaExibida !== "---") {
            const num = parseFloat(mediaExibida);
            bgMedia = num >= 6.0 ? "bg-green-900/20 text-green-400" : "bg-red-900/20 text-red-400";
        }

        // Criamos os TDs embutindo todas as notas nos atributos (data-m1, data-m2, etc)
        html += `<tr class="bg-slate-900/10 hover:bg-slate-900/50 transition-colors">
            ...
            <td class="media-final-col font-bold border border-slate-700 text-center py-2 ${bgMedia} transition-all duration-300"
                data-m1="${fmt1}" data-m2="${fmt2}" data-m3="${fmt3}" data-geral="${fmtGeral}">
                ${mediaExibida}
            </td>
        </tr>`;
    }
    ...
    if (selFoco && !selFoco.hasAttribute('data-fast-listener')) {
        ...
        // Remove listeners antigos (prevenção de conflitos)
        const novoSelect = selFoco.cloneNode(true);
        selFoco.parentNode.replaceChild(novoSelect, selFoco);
        
        novoSelect.addEventListener('change', (e) => {
            const val = e.target.value; // ex: '1', '2', 'geral'
            const cells = document.querySelectorAll('.media-final-col');
            
            cells.forEach(cell => {
                // Resgata o valor correspondente escondido no HTML
                const media = cell.getAttribute(val === 'geral' ? 'data-geral' : `data-m${val}`);
                cell.innerText = media;
                
                // Reaplica a estilização (Vazio azul / Aprovado verde / Reprovado vermelho)
                if (media === "---") {
                    cell.className = "media-final-col font-bold border border-slate-700 text-center py-2 bg-blue-900/10 text-blue-400 transition-all duration-300";
                } else {
                    const num = parseFloat(media);
                    const cor = num >= 6.0 ? 'bg-green-900/20 text-green-400' : 'bg-red-900/20 text-red-400';
                    cell.className = `media-final-col font-bold border border-slate-700 text-center py-2 ${cor} transition-all duration-300`;
                }
            });
        });
        
        novoSelect.setAttribute('data-fast-listener', 'true');
    }
...
async function renderBanner() {
    ...
    // APLICAÇÃO DINÂMICA DE SKINS DE BORDA COMPRADAS
    // Limpa efeitos anteriores conhecidos para evitar conflitos no re-render
    els.imgProfile.classList.remove('border-rainbow', 'border-valiriano', 'border-cyberpunk');
    if (currentUser.skinAtiva) {
        els.imgProfile.style.borderWidth = "0px"; // Permite que o outline/border do gradiente CSS atue livremente
        els.imgProfile.classList.add(currentUser.skinAtiva);
    } else {
        els.imgProfile.style.borderWidth = "4px";
        els.imgProfile.style.borderColor = currentUser.profileBorderColor || "#00e5ff";
    }

    // APLICAÇÃO DINÂMICA DE TEMAS NO CONTAINER DO PERFIL
    const cardPerfil = document.getElementById('dashboard-aluno');
    if (cardPerfil) {
        cardPerfil.classList.remove('theme-hacker', 'theme-arcano');
        if (currentUser.temaAtivo) {
            cardPerfil.classList.add(currentUser.temaAtivo);
        }
    }
...
window.mochilaAPI = {
    ...
    renderizarMochilaUI() {
        ...
        if (mochila.length === 0) {
            ...
            itemContainer.innerHTML = mochila.map(item => `
                <div class="bg-slate-950/40 border border-slate-700/50 p-4 rounded-xl flex items-center justify-between gap-4 group hover:border-blue-500/50 transition-all shadow-md">
                    ...
                    <button onclick="window.mochilaAPI.invocarConsumivel('${item.id}')" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-900/30">
                        Usar 1x
                    </button>
                </div>
            `).join('');
        ...
        const listaEstilosDisponiveis = [
            ...
            { id: 'borda_cyberpunk', nome: 'Neon Cyberpunk', classe: 'border-cyberpunk', tipo: 'skin' },
            { id: 'tema_hacker', nome: 'Terminal Hacker', classe: 'theme-hacker', tipo: 'tema' },
        ...
        if (estilosAdquiridos.length === 0) {
            estiloContainer.innerHTML = `
                <div class="flex flex-col items-center justify-center p-8 text-slate-500 italic text-xs">
                    <i class="fas fa-palette text-2xl mb-2 opacity-40"></i>
                    Nenhum estilo visual desbloqueado nesta jornada.
                </div>`;
        } else {
            estiloContainer.innerHTML = estilosAdquiridos.map(cos => {
                ...
                return `
                    <div class="bg-slate-950/40 border ${isActive ? 'border-emerald-500/30 bg-emerald-950/5' : 'border-slate-800'} p-3 rounded-xl flex items-center justify-between gap-4 transition-all">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-lg border-2 ${isSkin ? cos.classe : 'border-slate-700'} bg-slate-900 shrink-0 flex items-center justify-center text-[9px] font-black text-slate-500 uppercase">
                                ${isSkin ? 'Borda' : 'Tema'}
                            </div>
                            <div>
    ...
    async activarCosmetico(classeCSS, tipo) {
        try {
            const userRef = doc(db, "users", currentUser.uid);
            const updatePayload = tipo === 'skin' ? { skinAtiva: classeCSS } : { temaAtivo: classeCSS };
    ...
    async removerCosmetico(tipo) {
        try {
            const userRef = doc(db, "users", currentUser.uid);
            const updatePayload = tipo === 'skin' ? { skinAtiva: "" } : { temaAtivo: "" };
    ...
    async invocarConsumivel(itemId) {
        ...
        try {
            const userRef = doc(db, "users", currentUser.uid);
            let mochila = [...(currentUser.mochilaPedagogica || [])];
            ...
            if (idx > -1) {
                ...
                await addDoc(collection(db, "logs_pedagogicos"), {
                    ...
                    itemNome: nomeItem,
                    acao: "Ativação de Consumível Loja",
                    turma: currentUser.turma || "Sem Turma",
...
window.alunoTcgAPI = {
    ...
    renderGrid: () => {
        ...
        Object.entries(conceitosMap).forEach(([nomeConceito, variantes]) => {
            ...
            const isOwned = tipoEstado === 'owned';
            ...
            if (isOwned) {
                ...
                let dateStr = '';
                if (!isProf) {
                    const posse = minhasCartas.find(c => c.id === cartaParaExibir.id);
                ...
                statusHtml = `
                    ...
                    <div class="absolute bottom-0 left-0 w-full p-2.5 z-20 text-center">
                        <span class="bg-indigo-600/90 backdrop-blur-sm text-white text-[7px] sm:text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-md shadow-lg border border-indigo-400 block w-full truncate">${isProf ? 'Modo Mestre' : 'Conquistado: ' + dateStr}</span>
                    </div>
                `;
    ...
    criarModalDetalhes: () => {
        ...
        modalDiv.id = 'modal-tcg-detalhes';
                    ...
                    <div class="flex justify-center items-center">
                        <div id="modal-tcg-card-preview" class="w-72 sm:w-80 aspect-[2/3] relative rounded-2xl overflow-hidden shadow-2xl bg-slate-800">
                            ...
                            <div class="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black via-black/80 to-transparent p-4 pt-12 text-center">
                                <h3 id="modal-tcg-nome" class="text-white font-black text-base leading-tight drop-shadow-md">Nome</h3>
                            </div>
                        </div>
                    </div>
